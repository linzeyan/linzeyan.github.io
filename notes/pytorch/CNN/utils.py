...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        label = self.im_list[idx][:CAPTCHA]
...
def load_data(batch_size=4, max_m=-1, split_rate=0.2, gpu=True):
    ...
    folder = os.path.join(wd, IMG_FOLDER)
    imgs = [i for i in os.listdir(folder) if i.endswith(IMG_FORMAT) 
            and Image.open(folder + '/' + i).size == (IMG_WIDTH, IMG_LENGTH)]
    if not imgs:
