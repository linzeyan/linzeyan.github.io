...
export let options = {
	...
	tlsVersion: {
		// 表示允许在与服务器交互中使用的唯一 SSL/TLS 版本的字符串，或者一个指定允许使用的“最小”和“最大”版本的对象
		min: "tls1.2",
		max: "tls1.3",
	},
	tlsCipherSuites: [
		// 允许在与服务器的 SSL/TLS 交互中使用的密码套件列表。由于底层 go 实现的限制，不支持更改 TLS 1.3 的密码，并且不会执行任何操作
	...
	thresholds: {
		// 一组阈值规范，用于根据指标数据配置在何种条件下测试成功与否，测试通过或失败
		// 发出的请求数量需要大于1000
